...
function calcScore(
  ...
  if (mqDiff <= 0.05)                   s += 1;
  if (r.cat === cat)                     s += 1;
  if (hasAcc(r.garage)  === hasGarage)   s += 1;
  if (hasAcc(r.cantina) === hasCantina)  s += 1;
  return s;
}
...
function Badge({ value }: { value: boolean }) {
  return (
    <span className={[
      ...
      value ? "bg-green-50 text-green-700 border-green-200" : "bg-mi-hover text-mi-subtle border-mi-border",
    ].join(" ")}>
      {value ? "Sì" : "No"}
    </span>
  );
  ...
  return (
    <div>
      ...
      <p className="text-sm text-mi-muted">
        {loading ? "Caricamento dati in corso…" : "Transazioni simili al tuo immobile — prove oggettive per negoziare."}
      </p>
    </div>
  );
}
...
export default function CompsContent() {
  ...
  useEffect(() => {
    fetchAllPages<RawRow>("transazioni", "attoid,anno,mese,metratura,prezzo,zonaOMI,cat,garage,cantina")
      .then(({ data, error }) => {
  ...
  const categorieDisponibili = useMemo(() => Array.from(new Set(raw.map(r => r.cat).filter(Boolean))).sort(), [raw]);
  ...
  function handleTrova() {
    ...
    for (const r of raw) {
      ...
      const anno = Number(r.anno), mese = Number(r.mese);
      candidates.push({
        attoid: r.attoid,
    ...
    candidates.sort((a, b) => b.score !== a.score ? b.score - a.score : b.annoMese - a.annoMese);
  ...
  function buildSpiegazione(): string {
    ...
    if (stats.trendPct !== null) {
      if (stats.trendPct > 2)       t += `Le transazioni più recenti mostrano un trend in crescita (+${stats.trendPct}% rispetto alle più vecchie). `;
      else if (stats.trendPct < -2) t += `Le transazioni più recenti mostrano un trend in calo (${stats.trendPct}% rispetto alle più vecchie). `;
      else                          t += `Le transazioni più recenti mostrano prezzi stabili rispetto alle più vecchie. `;
    }
    return t;
  }
    ...
    <div className="p-4 md:p-8 max-w-[1100px] mx-auto space-y-6">
      ...
      <div className="bg-mi-card rounded-2xl border border-mi-border shadow-card p-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <div className="sm:col-span-2">
            <label className="block text-[12px] font-semibold text-mi-muted uppercase tracking-wider mb-1.5">Zona OMI</label>
            <select value={zona} onChange={e => setZona(e.target.value)}
              className="w-full h-10 px-3 pr-8 rounded-lg border border-mi-border bg-white text-sm font-medium text-mi-text appearance-none focus:outline-none focus:ring-2 focus:ring-mi-primary/20 focus:border-mi-primary transition-colors cursor-pointer"
              style={SELECT_STYLE}>
          ...
          <div>
            <label className="block text-[12px] font-semibold text-mi-muted uppercase tracking-wider mb-1.5">Metratura</label>
            <div className="relative">
              <input type="number" min="10" max="1000" value={mqInput} onChange={e => setMqInput(e.target.value)}
                placeholder="es. 70"
                className="w-full h-10 pl-3 pr-10 rounded-lg border border-mi-border bg-white text-sm font-medium text-mi-text placeholder:text-mi-subtle focus:outline-none focus:ring-2 focus:ring-mi-primary/20 focus:border-mi-primary transition-colors" />
              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[11px] text-mi-subtle pointer-events-none">mq</span>
            </div>
          </div>

          <div>
            <label className="block text-[12px] font-semibold text-mi-muted uppercase tracking-wider mb-1.5">Categoria catastale</label>
        ...
        <div className="mt-5">
          <button onClick={handleTrova}
            className="inline-flex items-center gap-2 h-10 px-6 rounded-xl bg-mi-primary text-white text-[14px] font-semibold hover:bg-mi-primary-dark active:scale-[0.98] transition-all duration-150 shadow-card">
      ...
      {comps && stats && (
        <div key={resultKey} className="space-y-5 animate-fade-in-up">
              ...
              <table className="w-full text-[13px] min-w-[720px]">
                <thead>
                  <tr className="border-b border-mi-border bg-white sticky top-0 z-10">
                    {["Data","Prezzo","Mq","€/mq","Categoria","Garage","Cantina","Match"].map(h => (
                      <th key={h} className="px-4 py-3 text-left text-[11px] font-semibold text-mi-subtle uppercase tracking-wider whitespace-nowrap first:pl-5 last:pr-5">
                        {h}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {comps.map((c, i) => {
                    ...
                    const rowBg = belowMedian ? "rgba(34,197,94,0.055)" : aboveMedian ? "rgba(239,68,68,0.055)" : "transparent";
                    return (
                      <tr key={c.attoid + i}
                        ...
                        <td className="px-4 py-2.5 text-mi-text whitespace-nowrap">
                          {fmt(c.mq)}
                          {c.stimato && <span title="Metratura stimata da vani" className="ml-1 text-amber-500 cursor-help">⚠</span>}
                        </td>
          ...
          <AnalysisCard text={buildSpiegazione()} />
