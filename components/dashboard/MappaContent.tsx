...
import { fmt } from "@/lib/utils/stats";
...
export default function MappaContent() {
  ...
  const cityStats = useMemo(() => {
    ...
    return { cityMean, cheapest, priciest };
  }, [zoneStats]);
  ...
  const spiegazione = useMemo(() => {
    ...
    if (selectedZona && zoneStats[selectedZona]) {
      ...
      const pos  = diff > 0 ? `il ${Math.abs(diff)}% sopra la media città` : diff < 0 ? `il ${Math.abs(diff)}% sotto la media città` : "in linea con la media città";
    ...
    const totalTx     = Object.values(zoneStats).reduce((s, v) => s + v.count, 0);
    return `Milano ha un prezzo medio di ${fmt(cityStats.cityMean)} €/mq su ${totalTx.toLocaleString("it-IT")} transazioni. Le zone più care sono ${priceyNames} (oltre ${fmt(cityStats.priciest[0][1].mean)} €/mq). Le più accessibili sono ${cheapNames} (sotto ${fmt(cityStats.cheapest[2][1].mean)} €/mq). Clicca su una zona per i dettagli.`;
  }, [selectedZona, zoneStats, cityStats]);

  const totalTx     = Object.values(zoneStats).reduce((s, v) => s + v.count, 0);
  const nZone       = Object.keys(zoneStats).length;
    ...
    <div className="p-4 md:p-8 max-w-[1100px] mx-auto space-y-6">
      ...
      <div>
        <div className="flex items-center gap-2.5 mb-1.5">
          ...
          <h1 className="text-[22px] font-semibold text-mi-text tracking-[-0.02em]">Mappa prezzi per zona OMI</h1>
        </div>
        <p className="text-sm text-mi-muted">
          {loading ? "Caricamento dati in corso…" : `Mappa interattiva · ${nZone} zone · ${totalTx.toLocaleString("it-IT")} transazioni`}
        </p>
      </div>
      ...
      <div className="bg-mi-card rounded-2xl border border-mi-border shadow-card p-4">
        {loading ? (
          ...
          <>
            ...
            <MappaLeaflet zoneStats={zoneStats} geojson={geojson} onZoneClick={setSelectedZona} />
          </>
        )}
      </div>

      {spiegazione && (
        <AnalysisCard title={selectedZona ? `Zona selezionata · ${selectedZona}` : "Panoramica Milano"} text={spiegazione}>
          {selectedZona && (
            <button onClick={() => setSelectedZona(null)} className="mt-3 text-[12px] text-mi-primary hover:underline">
